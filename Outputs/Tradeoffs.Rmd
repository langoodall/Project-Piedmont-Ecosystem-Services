...
AWBDf %>%
  aov(AWB ~ Prescription * RCP * Model, data = .) %>%
  summary(.)

ggplot(AWBDf, aes(x = AWB, fill = Model)) +
...
fruitTreeAllDf %>%
  ...
  ggplot(., aes(x = Time, y = Score, col = Model)) +
  geom_line(size = 2) +
  theme_classic() +
  facet_grid(RCP ~ Prescription)
...
fruitTreeAllDf %>%
  ...
  group_by(Time, RCP, Model, Prescription) %>%
  summarise(Score = mean(Score)) %>%
  ggplot(., aes(x = Time, y = Score, col = Prescription)) +
  geom_line(size = 2) +
  theme_classic() +
  facet_grid(RCP ~ Model)
...
fruitTreeAllDf %>%
  ...
  group_by(Time, RCP, Model, Prescription) %>%
  summarise(Score = mean(Score)) %>%
  ggplot(., aes(x = Time, y = Score, col = RCP)) +
  geom_line(size = 2) +
  theme_classic() +
  facet_grid(Prescription ~ Model)
...
aov.1 <- aov(Score ~ RCP * Model * Prescription, data = x)
summary(aov.1)
...
tradeoffDf_long %>%
  ...
  geom_bar(stat = "identity") +
  geom_text(aes(label = round(total, 1)), vjust = -0.3, color = "black") +
  theme_classic() +
  facet_grid(RCP ~ Prescription)
...
strDivDf %>%
  mutate(Model = sub("_[^_]+$", "", Model)) %>%
  filter(Time > 0) %>%
  aov(H ~ Prescription * Model * RCP, data = .) %>%
  summary(.)
...
df <- successionLogAllDf[,c(1,2,5,6,7,8,9,62:64)] %>%
  filter(Time < 81, Time > 0) %>%
  inner_join(strDivAllDf, by = c("Time", "Run", "RCP", "Model", "Prescription")) %>%
  rename(Structure_H = H) %>%
  inner_join(diversityAllDf, by = c("Time", "Run", "RCP", "Model", "Prescription")) %>%
...
columnPairs <- combn(colnames(df), 2, simplify = FALSE)
bagplots <- "C:/Users/lagoodal/Desktop/Dissertation Stuff/Chapter 2/Bagplots"

plot_files <- list()
for (pair in columnPairs) {
  x <- pair[1]
  y <- pair[2]
  filename <- paste0(bagplots, "/", x, "vs", y, ".png")
  plot_files[[paste(x, y, sep = "_")]] <- filename
  # png(filename, width = 1000, height = 1000)
  bag <- bagplot(df[, c(x, y)],
                 show.outlier = TRUE, 
                 show.whiskers = FALSE, 
                 col.loophull = "lightblue", 
                 col.baghull = "dodgerblue",
                 main = paste(x, "vs", y))
  
  x_median_plot <- bag$center[1]
  y_median_plot <- bag$center[2]
  
  abline(v = x_median_plot, col = "black", lty = 2)
  abline(h = y_median_plot, col = "black", lty = 2)
}
# dev.off()
...
n <- length(col_names)
plot_list <- vector("list", length = n * n)

for (i in 1:n) {
  for (j in 1:n) {
    index <- (i-1) * n + j
    if (i > j) {
      img_path <- plot_files[[paste(col_names[j], col_names[i], sep = "_")]]
      plot_list[[index]] <- rasterGrob(as.raster(png::readPNG(img_path)), interpolate = TRUE)
    } else {
      plot_list[[index]] <- nullGrob()
    }
  }
}

grid.arrange(grobs = plot_list, nrow = n, ncol = n)


# Get correlation data across all timesteps
corDf <- cor(df, use = "pairwise.complete.obs") %>%
  as.data.frame()
names <- sort(colnames(corDf))
corDf <- corDf[names,names]
corDf[upper.tri(corDf, diag = TRUE)] <- NA
corDf[corDf == 1] <- NA
# Get cumulative r scores to determine 
corDf <- corDf %>%
  summarise_all(sum, na.rm = TRUE) %>%
  rbind(corDf, .)
row.names(corDf)[12] <- "Total"


corDfFiltered <- corDf
corDfFiltered[corDfFiltered > 0.8 | corDfFiltered < -0.8] <- NA
corDfFiltered <- corDfFiltered[-12,] %>%
  summarise_all(sum, na.rm = TRUE) %>%
  `row.names<-`("Total") %>%
  rbind(corDfFiltered[-12,], .)


test <- corDfFiltered[-12,]
test[upper.tri(test, diag = TRUE)] <- NA

test <- unlist(test) %>% as.data.frame()



mean <- mean(test$., na.rm = TRUE)
sd <- sd(test$., na.rm = TRUE)

# Generate x values for normal distribution curve
xVals <- seq(min(test$., na.rm = TRUE), max(test$., na.rm = TRUE), length.out = 100)

# Compute density values
densityVals <- dnorm(xVals, mean = mean, sd = sd)

# Create data frame for the normal curve
densityDf <- data.frame(x = xVals, y = densityVals)

# Plot
ggplot(test, aes(x = .)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "lightblue", color = "black", alpha = 0.5) +  # Histogram
  geom_line(data = densityDf, aes(x = x, y = y), color = "red", size = 1) +  # Normal distribution curve
  geom_vline(xintercept = mean, color = "blue", linetype = "dashed", size = 1) +  # Mean line
  geom_vline(xintercept = mean + sd, color = "blue") +
  geom_vline(xintercept = mean + sd + sd, color = "blue") +
  geom_vline(xintercept = mean - sd, color = "blue") +
  geom_vline(xintercept = mean - sd - sd, color = "blue") +
  labs(title = "Distribution Curve with Mean and SD",
       x = "Value",
       y = "Density") +
  theme_minimal()
